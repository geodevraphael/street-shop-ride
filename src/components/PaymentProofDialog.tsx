  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="lg" className="gap-1.5"><CheckCircle2 className="h-4 w-4" /> Nimelipia</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader><DialogTitle>Thibitisha malipo</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div>
            <Label>Namba ya muamala (M-Pesa)</Label>
            <Input value={ref} onChange={(e) => setRef(e.target.value.toUpperCase())} placeholder="QABCDE12345" />
          </div>
          <div>
            <Label>Ambatisha SMS / risiti (picha)</Label>
            <Input type="file" accept="image/*" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
            <p className="mt-1 text-xs text-muted-foreground">Picha ya SMS au screenshot ya M-Pesa.</p>
          </div>
        </div>
        <DialogFooter>
          <Button onClick={submit} disabled={busy} className="gap-1.5">
            <Upload className="h-4 w-4" />{busy ? "Inatuma…" : "Tuma uthibitisho"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
