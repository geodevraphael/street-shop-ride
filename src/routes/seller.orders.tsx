...
type Action = { label: string; nextStatus?: OrderStatus; variant?: "default" | "outline" };
const ACTION: Partial<Record<OrderStatus, Action>> = {
  placed: { label: "Kubali oda", nextStatus: "accepted", variant: "default" },
  accepted: { label: "Subiri proof", variant: "outline" },
...
function SellerOrders() {
  ...
  const needsAction = useMemo(
    () => orders.filter((o) => o.status === "placed" || o.status === "payment_submitted").length,
    [orders],
  );
    ...
    <div className="space-y-4">
      {needsAction > 0 && (
        <div className="flex items-center gap-2 rounded-xl border bg-warning/10 p-3 text-sm">
          ...
          <span>
            <b>{needsAction}</b> oda zinahitaji hatua yako sasa hivi.
          </span>
        </div>
      )}
...
function Row({ o, onChanged }: { o: Order; onChanged: () => void }) {
  ...
  return (
    ...
    >
      <Link to="/orders/$orderId" params={{ orderId: o.id }} className="min-w-0 flex-1">
